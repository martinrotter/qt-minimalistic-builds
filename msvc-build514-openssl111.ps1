...
$openssl_base_folder = "C:\openssl111"
...
gci -r -include "*.prl" $prefix_folder | foreach-object { $a = $_.fullname; ( get-content $a ) | foreach-object { $_ -replace "C:\\\\openssl111\\\\lib", '$$$$[QT_INSTALL_LIBS]\\' } | set-content $a }

# Create final archive.
& "$tools_folder\7za.exe" a -t7z "${prefix_base_folder}-openssl111a.7z" "$prefix_folder" -mmt -mx9
